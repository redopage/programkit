...
each output. For the manual half of EMB-15, paste the styled-script and basic-HTML snippets into a
page on another origin and confirm search, filters, and detail drawers remain interactive. Import
the iCal URL into a calendar app and verify the published titles, dates, times, and rooms. These are
delivery checks rather than unimplemented product state.
