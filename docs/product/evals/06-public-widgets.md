...
| EMB-15 | Verified | Schedule studio's Share program dialog offers all five views, hosted-link and iframe outputs, JSON, XML, and iCal feeds, track and room filters, description visibility, and an accent-color control. Generated feed URLs are CORS-enabled and independently select the event. Cross-origin iframe rendering and calendar import remain manual browser checks. |
...
From Schedule studio, open **Share program** and exercise each output. For the iframe half of
EMB-15, paste the generated snippet into a page on another origin and confirm search, filters, and
detail drawers remain interactive. Import the iCal URL into a calendar app and verify the published
titles, dates, times, and rooms. These are delivery checks rather than unimplemented product state.
