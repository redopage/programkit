...
| SPK-08 | Partial        | Bio changes round-trip between the portal and organizer view and automatically satisfy the bio requirement. Binary headshot upload is the remaining gap.                                                                             |
...
| SPK-10 | Not yet        | The shared binary upload and organizer download surface is the next implementation slice.                                                                                                                                            |
...
The highest-value remaining work is one shared upload pipeline for profile headshots and requested
deliverables. It should store bytes in R2, metadata in the event Durable Object, expose participant-
scoped upload and organizer-scoped download endpoints, and reuse the same asset record from speaker,
content-management, submission, and session surfaces.
