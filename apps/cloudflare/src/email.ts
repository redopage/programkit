...
export interface ActionLinksEmailInput {
  title: string
  intro: string
...
export function actionLinksEmail(input: ActionLinksEmailInput) {
  const title = escapeHtml(input.title)
  const intro = escapeHtml(input.intro)
  const footnote = escapeHtml(input.footnote)
  const textActions = input.actions.map((action) => `${action.label}: ${action.url}`).join('\n\n')
  const htmlActions = input.actions
    .map(
      (action) =>
        `<tr><td style="padding:0 0 12px"><a href="${escapeHtml(action.url)}" style="display:block;padding:11px 14px;font-size:15px;line-height:20px;font-weight:600;color:#18181b!important;text-decoration:none;border:1px solid #e4e4e7;border-radius:12px">${escapeHtml(action.label)}</a></td></tr>`,
    )
    .join('')

  return {
    text: `${input.title}\n\n${input.intro}\n\n${textActions}\n\n${input.footnote}`,
    html: `<!doctype html><html><body style="margin:0;background:#ffffff;color:#18181b"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td align="center" style="padding:40px 20px"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:480px"><tr><td style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;line-height:1.5"><h1 style="margin:0 0 12px;font-size:24px;line-height:1.2;font-weight:650;letter-spacing:-0.02em;color:#18181b">${title}</h1><p style="margin:0 0 24px;font-size:16px;color:#52525b">${intro}</p><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">${htmlActions}</table><p style="margin:12px 0 0;font-size:13px;color:#71717a">${footnote}</p></td></tr></table></td></tr></table></body></html>`,
  }
}
