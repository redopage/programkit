...
function nativeDateTimeParts(iso: string, timeZone: string) {
...
function nativeSessionFormat(format: Session['format']): AcceleventsSessionPayload['format'] {
  switch (format) {
    case 'keynote':
      return 'MAIN_STAGE'
    case 'workshop':
...
export function buildAcceleventsExportItems(
  ...
  const sessionItems: AcceleventsExportItem[] = plan.release.placements.map((placement) => {
    ...
    const payload: AcceleventsSessionPayload = {
      ...
      startTime: nativeDateTimeParts(placement.startsAt, plan.event!.timezone),
      endTime: nativeDateTimeParts(placement.endsAt, plan.event!.timezone),
      location: room.name,
      format: nativeSessionFormat(session.format),
      status: 'VISIBLE',
