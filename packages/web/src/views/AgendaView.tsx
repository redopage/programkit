...
export function AgendaView({ navigate }: { navigate: (to: string) => void }) {
  ...
  const published = publicAgenda(state)
  const speakerCount = new Set(
    published.flatMap((item) => item.speakers.map((speaker) => speaker.id)),
  ).size
  const agenda = published.filter((item) => trackId === 'all' || item.track?.id === trackId)
