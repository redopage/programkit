...
import type { ChangeSet } from '@crm-library/core'

import { useWorkspace } from '../lib/workspace.tsx'
import { Button, Drawer, EmptyState, PageHeader, StatusBadge } from '../components/ui.tsx'
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        eyebrow="Safe agency"
        title="Change review"
        description="Human and agent proposals are validated, explained, approved, and committed as explicit transactions."
      />
      ...
      <div className="flex flex-col gap-6">
        ...
        {changeSet.warnings.length > 0 ? (
          <div className="rounded-xl bg-amber-50 p-4 ring-1 ring-amber-900/10">
            <p className="text-base font-medium text-amber-900 sm:text-sm">Before you approve</p>
            <ul role="list" className="flex flex-col gap-1 pt-1">
              {changeSet.warnings.map((warning) => (
                <li key={warning} className="text-pretty text-base text-amber-800 sm:text-sm">
                  {warning}
                </li>
              ))}
            </ul>
