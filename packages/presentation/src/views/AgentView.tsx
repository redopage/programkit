...
import { readinessSummary } from '@crm-library/core'
...
export function AgentView({ navigate }: { navigate: (to: string) => void }) {
  ...
  return (
    <div className="flex flex-col gap-8">
      <PageHeader
        eyebrow="MCP and procedural skills"
        title="Agent workspace"
        description="The agent reads the same operational truth, drafts through the same capabilities, and cannot bypass server-enforced approval boundaries."
      />
        ...
        >
          <label className="min-w-0 flex-1">
            ...
            <input
              ...
              className="focus-visible:outline-emerald-400 min-h-11 w-full rounded-lg bg-white/10 px-3 py-2 text-base text-white ring-1 ring-white/15 placeholder:text-zinc-500 focus-visible:outline-2 -outline-offset-1 sm:min-h-9 sm:text-sm"
            />
          </label>
      ...
      <div className="grid gap-8 xl:grid-cols-[7fr_5fr]">
        <section aria-labelledby="brief-heading">
          <div className="border-b border-zinc-950/5 pb-3">
            <h2 id="brief-heading" className="text-lg font-semibold text-zinc-950">
          ...
          <dl className="divide-y divide-zinc-950/5">
            {[
              ...
              <div key={label} className="flex items-baseline gap-4 py-4">
                <dt className="min-w-0 flex-1 text-base font-medium text-zinc-950 sm:text-sm">
                  {label}
                </dt>
                <dd className="text-2xl font-semibold tracking-tight tabular-nums text-zinc-950">
                  {value}
                </dd>
                <dd className="hidden w-48 text-right text-sm text-zinc-500 sm:block">{detail}</dd>
              </div>
            ))}
          </dl>
        </section>
        ...
        <section aria-labelledby="policy-heading">
          <div className="border-b border-zinc-950/5 pb-3">
            <h2 id="policy-heading" className="text-lg font-semibold text-zinc-950">
      ...
      <section aria-labelledby="skills-heading">
        <div className="border-b border-zinc-950/5 pb-3">
          <h2 id="skills-heading" className="text-lg font-semibold text-zinc-950">
            Packaged skills
          </h2>
          <p className="text-base text-zinc-500 sm:text-sm">
