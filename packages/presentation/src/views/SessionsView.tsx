import { CalendarDaysIcon, MagnifyingGlassIcon } from '@heroicons/react/16/solid'
import { useState } from 'react'

import { useWorkspace } from '../lib/workspace.tsx'
import { eventDateTime } from '../lib/date.ts'
import { Avatar, Button, Drawer, PageHeader, TrackBadge, cx } from '../components/ui.tsx'
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        title="Sessions"
        description="Sessions stay independent from people, participation, and schedule placement so each can change safely."
        actions={
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0 overflow-x-auto">
          <div className="flex w-max gap-1" role="group" aria-label="Track filters">
            {[{ id: 'all', name: 'All tracks' }, ...state.tracks].map((track) => (
              <button
                ...
                className={cx(
                  'focus-ring min-h-9 rounded-lg px-3 text-base text-zinc-600 sm:min-h-7 sm:text-sm',
                  trackId === track.id && 'bg-zinc-100 text-zinc-950',
                )}
              ...
              >
                <span className="flex items-start justify-between gap-3">
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block text-base capitalize text-zinc-500">
                      {session.format} · {session.durationMinutes} min
                    </span>
                  </span>
                  <TrackBadge name={track.name} color={track.color} />
                </span>
    ...
    <Drawer open={open} onClose={onClose} title={session.title}>
      <div className="flex flex-col gap-6">
        <div className="flex items-center gap-2">
          <TrackBadge name={track.name} color={track.color} />
          <span className="text-base capitalize text-zinc-500 sm:text-sm">
            {session.format} · {session.durationMinutes} min
          </span>
        </div>
